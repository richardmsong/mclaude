...
func newTestServer(t *testing.T) *Server {
	...
	return NewServer(nil, accountKP, "nats://localhost:4222", "", 8*time.Hour, "test-admin-token", nil, nil, "", "")
}
...
func TestHandleRefresh_ValidJWT(t *testing.T) {
	...
	srv := NewServer(nil, accountKP, "nats://localhost:4222", "", 8*time.Hour, "admin", nil, nil, "", "")
...
func TestHandleRefresh_WrongAccountKey(t *testing.T) {
	...
	srv := NewServer(nil, accountB, "nats://localhost:4222", "", time.Hour, "admin", nil, nil, "", "")
...
func TestAuthMiddleware_ValidToken_InjectsUserID(t *testing.T) {
	accountKP, _ := nkeys.CreateAccount()
	srv := NewServer(nil, accountKP, "nats://test", "", time.Hour, "admin", nil, nil, "", "")
...
func TestAdminAuthMiddleware_Enforced(t *testing.T) {
	srv := NewServer(nil, mustAccountKP(t), "nats://test", "", time.Hour, "secret-admin-token", nil, nil, "", "")
	called := false
