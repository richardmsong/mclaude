...
| `mclaude.users.{uslug}.hosts.{hslug}.projects.{pslug}.api.sessions.create` | SPA, daemon | session-agent (JetStream, cmd consumer; publish + KV-watch, no reply) | `{name, branch, cwd, joinWorktree, extraFlags, permPolicy, allowedTools, quotaMonitor, requestId}` — `allowedTools` is an optional string array for `allowlist`/`strict-allowlist` policies; success: session appears in KV; error: `api_error` event on `events._api` |
