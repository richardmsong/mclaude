...
| `nkey_public` | yes | Ed25519 public key generated by the host controller at startup. Starts with `U`. CP stores it for future HTTP challenge-response authentication. |

CP validates the user's identity, creates the host in Postgres (slug, name, type, user_id, public_key). No JWT in the response — the host controller authenticates itself via HTTP challenge-response (see "Unified HTTP Credential Protocol"). `_` is a sentinel token that can never collide with real slugs (slugs are `[a-z0-9-]+`).
...
| `nkey_public` | yes | Ed25519 public key generated by the agent at startup. Starts with `U`. |
...
| Field | Description |
|-------|-------------|
| `nkey_public` | NKey public key (starts with `U`). CP looks up this key in Postgres to determine identity type. |
