...
| Job | `{release}-minio-bucket` | Post-install/post-upgrade hook. `restartPolicy: Never`, `activeDeadlineSeconds: {{ minio.bucketJobDeadlineSeconds }}` (default 300), `backoffLimit: 3`, `hook-delete-policy: before-hook-creation,hook-succeeded`. Image: `minio/mc:RELEASE.2025-04-16T18-25-19Z` via `mclaude-cp.image` helper. Both containers share an `emptyDir` volume named `mc-config` mounted at `/root/.mc` (init writes alias config; main container reads it). Init container: `mc alias set local http://{release}-minio:9000 $ACCESS_KEY $SECRET_KEY`, then loops on `mc ready local` (max 60s). Main container: `mc mb --ignore-existing local/{minio.bucket}`. Both security context helpers (`mclaude-cp.podSecurityContext`, `mclaude-cp.securityContext`) are intentionally omitted — the `minio/mc` image may run as root, and this is a one-time admin operation. **Pre-pull (ADR-0070, ADR-0071):** `deploy-main.yml` runs `docker pull minio/mc:<tag>` (short form — no `docker.io/` prefix; the self-hosted runner's Docker daemon cannot resolve the explicit prefix) + `k3d image import minio/mc:<tag> -c mclaude-dev` before `helm upgrade`. k3d normalises the short name to `docker.io/minio/mc:<tag>` in containerd so the pod finds it via `imagePullPolicy: IfNotPresent`. If `docker pull` or `k3d image import` fails the deploy step aborts before helm upgrade runs, so the hook never executes. If the hook still exceeds `bucketJobDeadlineSeconds`, it fails with `context deadline exceeded` — check k3d node resources and network connectivity. |
...
| `minio.mcImage.tag` | `RELEASE.2025-04-16T18-25-19Z` | `minio/mc` image tag. **When bumping this tag, also update the matching image reference in the `deploy-main.yml` pre-pull step** — the two must stay in sync (ADR-0070). |
| `minio.mcImage.pullPolicy` | `IfNotPresent` | `minio/mc` image pull policy. |
| `minio.bucketJobDeadlineSeconds` | `300` | `activeDeadlineSeconds` for the minio-bucket Job. Increased from the original 120s to allow time for a cold image pull in environments where the `minio/mc` image is not pre-cached (ADR-0070). |
...
| `values-k3d-ghcr.yaml` (cp) | Local k3d preview with ghcr.io images. Traefik ingress with TLS on `*.mclaude.richardmcsong.com`. Sets `ingress.minioHost: minio.mclaude.richardmcsong.com` (direct subdomain of the wildcard cert, covered by `*.mclaude.richardmcsong.com`). Overrides `minio.resources.requests.memory: 32Mi` (down from default 128Mi) so MinIO can schedule on the shared k3d node whose memory request budget is consumed by session-agent pods (ADR-0068). |
