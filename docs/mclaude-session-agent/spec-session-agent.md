...
2. **In-flight background shell tracking (K8s mode; two-phase):** **Not yet implemented** — the `pendingShells` and `inFlightShells` maps, the two-phase promotion logic, and the graceful shutdown shell-killed notifications (step 6) are all absent from code. Only `inFlightBackgroundAgents` (agent tracking) is implemented. Spec description below is the target design:
