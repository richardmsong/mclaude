...
5. Control-plane auto-detects provider type **sequentially, first match wins**: calls `{baseUrl}/api/v3/user` with the token — if 200, type is `github`. Otherwise calls `{baseUrl}/api/v4/user` — if 200, type is `gitlab`. If **any** attempt returned 401/403, returns `400` with `"invalid token — check that the token has at least read access"` (auth errors take priority — a 401 from one endpoint proves the provider was reached but the token is bad, even if the other endpoint returned 404). If all attempts returned only connection errors/404, returns `400` with `"could not reach provider — check the base URL"`. (In the unlikely case both succeed, GitHub wins — acceptable since no real GitLab instance responds to `/api/v3/user`.)
...
| Token exchange fails | Provider returns non-200 on code→token exchange, OR returns 200 with `{"error":"..."}` in body (GitHub always returns 200, even for errors like `bad_verification_code` or `redirect_uri_mismatch`) | `exchangeCode` must parse `error` and `error_description` fields from the response body. When `error` is non-empty, return it as the error (not "empty access_token"). Log includes the provider's error string. Redirects to return_url with `error=exchange_failed`. User sees toast: "Failed to connect — try again." No token obtained, no DB row created. |
