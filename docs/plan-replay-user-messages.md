...
2. If event has `uuid` and a matching pending message exists **OR** if event has no uuid but text content matches a pending message (defensive fallback): remove from `_pendingMessages`, splice the matching optimistic turn out of `_conversation.turns`, clear its `pendingUuid`, and re-insert it at the correct inline position — before the fresh streaming assistant turn under the same `parentToolUseId` if one exists, otherwise append at the end. Any still-pending optimistic turns always remain at the tail of `_conversation.turns` so un-echoed messages stay visible at the bottom of the chat.
...
4. If no matching pending turn was found (step 2 returned -1): create a new user turn and insert it inline using the same positioning rule as step 2 (before the fresh streaming assistant turn if one exists, otherwise append).
5. Text-matching fallback (step 2): retained as a defensive measure for the case where Claude's echo arrives without a uuid. It matches by exact string comparison of text content. Only fires when `event.uuid` is absent — uuid matching always takes priority.
...
`uuid` is optional for backward compatibility. If absent, the text-matching fallback fires: if the echo text matches a pending message, the existing optimistic turn is confirmed. If no match, a new inline turn is created.
...
- Remove old `addUserTurn()` (replaced by `addPendingMessage` which inserts an optimistic turn)
- Retain text-matching fallback dedup as defensive measure (fires only when uuid is absent)
