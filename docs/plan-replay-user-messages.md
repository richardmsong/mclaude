...
Add to `StreamJsonEvent` union (or the user event type):

```typescript
// On user events from --replay-user-messages
uuid?: string
isReplay?: boolean
isSynthetic?: boolean
```
...
1. If event has `uuid` and a matching pending message exists: remove from `_pendingMessages`
2. If event content is a tool_result: attach to matching ToolUseBlock (existing behavior, unchanged)
3. If event has `isSynthetic: true`: create a system turn with a system-message block
...
**Pending messages**: Render `eventStore.pendingMessages` at the bottom of the message list, after all conversation turns. Styled with reduced opacity (0.5) and a subtle "sending..." indicator.
...
**System messages from synthetic replays**: Render as gray, smaller text — similar to compaction summary blocks.
