...
2. If event has `uuid` and a matching pending message exists **OR** if event has no uuid but text content matches a pending message (defensive fallback): remove from `_pendingMessages` and clear `pendingUuid` on the matching optimistic turn in `_conversation.turns` — the turn stays at its position, just un-dimmed.
...
4. If no matching pending turn was found (step 2 returned -1): create a new user turn inline at current position
