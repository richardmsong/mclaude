...
- OAuth tokens for GitHub/GitLab are managed by the existing OAuth connection flow (see `plan-github-oauth.md`). The skill uses the connection's `gh`/`glab` CLI credentials to create repos and push.
