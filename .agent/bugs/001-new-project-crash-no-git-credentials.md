...
The real fix is the github-oauth feature (`docs/plan-github-oauth.md`) which provisions `gh auth` credential helpers via K8s Secrets. That feature's control-plane endpoints are not yet implemented (0/17 spec items).
...
- `docs/plan-github-oauth.md` — complete spec for OAuth credential flow (design audit: CLEAN)
