...
describe('LifecycleStore', () => {
  ...
  describe('start / stop', () => {
    it('subscribes to the correct lifecycle subject', () => {
      ...
      mockNats.simulateReceive(
        'mclaude.users.user-1.hosts.local.projects.project-1.sessions.session-1.lifecycle.started',
  ...
  describe('onLifecycleEvent', () => {
    ...
    it('matches the wildcard subject for any session-id suffix', () => {
      ...
      mockNats.simulateReceive(
        'mclaude.users.user-1.hosts.local.projects.project-1.sessions.session-abc.lifecycle.started',
        { type: 'session_created', sessionId: 'session-abc' } as unknown as LifecycleEvent,
      )
      mockNats.simulateReceive(
        'mclaude.users.user-1.hosts.local.projects.project-1.sessions.session-xyz.lifecycle.stopped',
    ...
    it('silently ignores malformed JSON events', () => {
      ...
      mockNats.publish('mclaude.users.user-1.hosts.local.projects.project-1.sessions.session-1.lifecycle.started', new TextEncoder().encode('not-json'))
