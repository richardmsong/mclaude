...
describe('TerminalVM', () => {
  ...
  describe('createTerminal', () => {
    it('requests terminal creation via terminal.create and returns terminalId', async () => {
      mockNats.requestHandlers.set(
        'mclaude.users.user-1.hosts.local.projects.project-1.terminal.create',
    ...
    it('publishes to terminal.create with cwd', async () => {
      mockNats.requestHandlers.set(
        'mclaude.users.user-1.hosts.local.projects.project-1.terminal.create',
      ...
      const req = mockNats.requests.find(r => r.subject === 'mclaude.users.user-1.hosts.local.projects.project-1.terminal.create')
    ...
    it('subscribes to terminal output after creation', async () => {
      ...
      const outputSubject = `mclaude.users.user-1.hosts.local.projects.project-1.terminal.${id}.output`
  ...
  describe('deleteTerminal', () => {
    it('requests terminal deletion via terminal.delete', async () => {
      mockNats.requestHandlers.set(
        'mclaude.users.user-1.hosts.local.projects.project-1.terminal.create',
      ...
      const req = mockNats.requests.find(r => r.subject === 'mclaude.users.user-1.hosts.local.projects.project-1.terminal.delete')
      expect(req).toBeDefined()
  ...
  describe('sendInput', () => {
    it('publishes raw bytes to terminal.{termId}.input', () => {
      ...
      const pub = mockNats.published.find(p => p.subject === 'mclaude.users.user-1.hosts.local.projects.project-1.terminal.term-1.input')
  ...
  describe('resize', () => {
    it('publishes resize event to terminal.resize', () => {
      vm.resize('term-1', 24, 80)

      const pub = mockNats.published.find(p => p.subject === 'mclaude.users.user-1.hosts.local.projects.project-1.terminal.resize')
      expect(pub).toBeDefined()
  ...
  describe('onOutput', () => {
    it('delivers output to all registered listeners for that terminal', async () => {
      ...
      mockNats.simulateReceive(`mclaude.users.user-1.hosts.local.projects.project-1.terminal.${id}.output`, 'data')
