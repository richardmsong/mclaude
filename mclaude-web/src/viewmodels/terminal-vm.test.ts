...
describe('TerminalVM', () => {
  ...
  describe('createTerminal', () => {
    it('requests terminal creation via api.terminal.create and returns terminalId', async () => {
      mockNats.requestHandlers.set(
        'mclaude.user-1.project-1.api.terminal.create',
    ...
    it('publishes to mclaude.{userId}.{projectId}.api.terminal.create with cwd', async () => {
      ...
      const req = mockNats.requests.find(r => r.subject === 'mclaude.user-1.project-1.api.terminal.create')
    ...
    it('subscribes to terminal output after creation', async () => {
      ...
      const outputSubject = `mclaude.user-1.project-1.terminal.${id}.output`
  ...
  describe('deleteTerminal', () => {
    it('requests terminal deletion via api.terminal.delete', async () => {
      ...
      const req = mockNats.requests.find(r => r.subject === 'mclaude.user-1.project-1.api.terminal.delete')
      expect(req).toBeDefined()
  ...
  describe('sendInput', () => {
    it('publishes raw bytes to terminal.{termId}.input', () => {
      ...
      const pub = mockNats.published.find(p => p.subject === 'mclaude.user-1.project-1.terminal.term-1.input')
  ...
  describe('resize', () => {
    it('publishes resize event to api.terminal.resize', () => {
      ...
      const pub = mockNats.published.find(p => p.subject === 'mclaude.user-1.project-1.api.terminal.resize')
      expect(pub).toBeDefined()
  ...
  describe('onOutput', () => {
    it('delivers output to all registered listeners for that terminal', async () => {
      ...
      mockNats.simulateReceive(`mclaude.user-1.project-1.terminal.${id}.output`, 'data')
